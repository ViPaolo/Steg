"""This module allows to perform encoding and decoding of text messages into an Image."""
# Module docstring
try:
    import numpy as np
...
def encode_image(img: Image.Image, txt : str) -> Image.Image:
    ...
    Parameters:
        ...
        The length of the text must not be larger than the HxWx3.
        ...
        for w in range(width):
            if bit_count >= len(txt_bits):
                ...
                if bit_count < len(txt_bits):
                    ...
                    # original pixel channel OR original bit (00000001 or 00000000), if the i-th bit is zero,
                    # then the code will start to make them appear.
...
def decode_image(img : Image.Image) -> str:
    ...
    Note that it may output valid strings although the image had not been encoded yet.
    Use only with images you can verify have been encoded.
...
def psnr(original_img, new_img):
    ...
    Returns the Peak-to-Signal Noise Ratio between two images.

    Parameters:
        original_img: The original image.
        new_igm: The new compressed or modified image.

    Return:
        psnr: the psnr as a float number.
    """
    ...
    #psnr formula
    psnr_r = 20 * np.log10(s / np.sqrt(mse))
    return psnr_r